...
def confidence_interval(N_samples=100, sample_size=30, true_mean=67, true_std=10, confidence=0.95):
    ...
    alpha = 1-confidence
    z = norm.ppf(1-alpha/2) #norm.isf(alpha)
    ...
    for i in range(N_samples):
        ...
        lower = sample_mean-sample_se*z/np.sqrt(sample_size)
        upper = sample_mean+sample_se*z/np.sqrt(sample_size)
        # this code block ends here
        ...
        if lower>true_mean or true_mean>upper:
            misses += 1
        # this code block ends here
    ...
    for i, (low, up) in enumerate(zip(ci_lowers, ci_uppers)):
        ...
        color = 'red' if low<true_mean<up else 'blue'
        # this code block ends here
